...
public class GuiHandTelescope extends GuiWHScreen implements GuiSkyScreen {
    ...
    public void onGuiClosed() {
        ...
        // 1.7.10: updateKeyBindState() doesn't exist, keybind state is handled elsewhere
    ...
    private void handleMouseMovement(float pticks) {
        ...
        if (grabCursor && !ctrl) {
            // 1.7.10: updateKeyBindState() doesn't exist
            ClientUtils.grabMouseCursor();
        ...
        if (!ctrl) {
            ...
            handleHandMovement(WrapMathHelper.floor(movementX), WrapMathHelper.floor(movementY));
        }
    }
    ...
    // 1.7.10: Return nullable Map instead of Optional
    private Map<StarLocation, Rectangle> drawCellEffect(int offsetX, int offsetY, int width, int height,
        ...
        for (StarPosition stars : usedStars) {
            ...
            drawRect(WrapMathHelper.floor(offsetX + stars.x), WrapMathHelper.floor(offsetY + stars.y), 5, 5);
        ...
        if (topFound != null) {
            ...
            if ((Math.abs(diffYaw) <= sFactor || Math.abs(playerYaw + 360F) <= sFactor)
                ...
                rectangles = RenderConstellation.renderConstellationIntoGUI(
                    topFound,
                    offsetX + wPart + WrapMathHelper.floor((diffYaw / sFactor) * width),
                    offsetY + hPart + WrapMathHelper.floor((diffPitch / sFactor) * height),
                    zLevel,
        ...
        GL11.glPopAttrib();
        return rectangles; // 1.7.10: Return nullable value instead of Optional
    }
    ...
    // 1.7.10: @Override removed - mouseReleased doesn't exist in GuiScreen

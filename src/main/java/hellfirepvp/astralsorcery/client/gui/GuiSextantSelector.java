...
public class GuiSextantSelector extends GuiWHScreen implements GuiSkyScreen {
    ...
    // REMOVED: private final EnumHand usedHand // 1.7.10 does not have EnumHand;
        ...
        for (SextantFinder.TargetObject to : this.availableTargets) {
            if (!this.showupTargets.containsKey(to)) {
                BlockPos target = UISextantCache.queryLocation(
                    new BlockPos(Minecraft.getMinecraft().thePlayer),
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        // 1.7.10: ScaledResolution needs 3 parameters
    ...
    private Map<Rectangle.Double, SextantFinder.TargetObject> drawCellWithEffects(float partialTicks, boolean canSeeSky,
        ...
        for (Map.Entry<SextantFinder.TargetObject, Tuple<BlockPos, Integer>> visibleTarget : this.showupTargets
            ...
            float alphaShowup = WrapMathHelper
            ...
            double offsetX = guiLeft + wPart - WrapMathHelper.floor((diffYaw / xsFactor) * width);
            double offsetY = guiTop + hPart - WrapMathHelper.floor((diffPitch / ysFactor) * height);
            ...
            for (Rectangle.Double r : this.selectMap.keySet()) {
                if (r.contains(mouse)) {
                    RenderingUtils.renderBlueTooltip(
                        ...
                        mouse.y,
                        Lists.newArrayList(
                            EnumChatFormatting.GOLD.toString() + I18n.format(
                                I18n.format(
    ...
    private void handleMouseMovement(float pticks) {
        ...
        if (!ctrl) {
            ...
            // 1.7.10: turn() doesn't exist, use setAngles()
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state) {
        ...
        if (state == 0 && this.dragging != null) {
            ...
            if (sel != null && (to = this.dropMap.get(sel)) != null && to.equals(this.dragging)) {
                PacketChannel.CHANNEL.sendToServer(new PktSetSextantTarget(to));

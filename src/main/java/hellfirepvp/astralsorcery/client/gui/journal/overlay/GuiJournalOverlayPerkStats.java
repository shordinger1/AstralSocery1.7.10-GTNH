...
public class GuiJournalOverlayPerkStats extends GuiScreenJournalOverlay {
    ...
    public void initGui() {
        ...
        for (hellfirepvp.astralsorcery.common.constellation.perk.attribute.PerkAttributeType t : AttributeTypeRegistry
            .getTypes()) {
            if (t instanceof VanillaAttributeType) {
    ...
    private void drawHeader() {
        ...
        List<String> split = fr.listFormattedStringToWidth(locTitle, WrapMathHelper.floor(HEADER_WIDTH / 1.4));
        int step = 14;
    ...
    private void drawPageText(int mouseX, int mouseY) {
        ...
        for (PerkStatistic stat : statistics) {
            ...
            Minecraft.getMinecraft().fontRenderer
                .drawString(stat.getPerkValue(), offsetX + nameStrWidth, offsetY + (line * 10), 0xEE333333);

            int strLength = Minecraft.getMinecraft().fontRenderer.getStringWidth(stat.getPerkValue());
            ...
            if (!stat.getSuffix()
                ...
                // 1.7.10: drawString doesn't have shadow parameter
                Minecraft.getMinecraft().fontRenderer
                    .drawString(stat.getSuffix(), offsetX + 25, offsetY + (line * 10), 0xEE333333);
            }
            line++;
        }
    ...
    private void drawCalculationDescription(int x, int y, PerkStatistic stat) {
        ...
        RenderingUtils.renderBlueTooltip(x, y, information, Minecraft.getMinecraft().fontRenderer);
    }
    ...
    private void buildDisplayWidth() {
        ...
        for (PerkStatistic stat : this.statistics) {
            int nameWidth = Minecraft.getMinecraft().fontRenderer
                .getStringWidth(I18n.format(stat.getUnlocPerkTypeName()));
            int valueWidth = Minecraft.getMinecraft().fontRenderer.getStringWidth(stat.getPerkValue());
            int suffixWidth = Minecraft.getMinecraft().fontRenderer.getStringWidth(stat.getSuffix());

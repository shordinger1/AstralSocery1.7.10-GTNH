...
public class GuiProgressionClusterRenderer {
    ...
    private void renderNodeToGUI(ResearchNode node, double lowerPosX, double lowerPosY, float zLevel) {
        ...
        net.minecraft.client.renderer.entity.RenderItem ri = RenderItemAccessor.getRenderItem();
        ...
        switch (node.getRenderType()) {
            ...
            case TEXTURE:
                ...
                tex.bindTexture();
                t.addVertexWithUV(pxWH, zoomedWH - pxWH, zLevel, of.x, of.y + tex.getVWidth());
                t.addVertexWithUV(
                    zoomedWH - pxWH,
                    zoomedWH - pxWH,
                    zLevel,
                    of.x + tex.getUWidth(),
                    of.y + tex.getVWidth());
                t.addVertexWithUV(zoomedWH - pxWH, pxWH, zLevel, of.x + tex.getUWidth(), of.y);
                t.addVertexWithUV(pxWH, pxWH, zLevel, of.x, of.y);
                t.draw();
                GL11.glColor4f(1F, 1F, 1F, 1F);
                TextureHelper.refreshTextureBindState();
            ...
            case TEXTURE_SPRITE:
                ...
                t.addVertexWithUV(pxWH, zoomedWH - pxWH, zLevel, uvTexture.key, uvTexture.value + res.getVLength());
                t.addVertexWithUV(
                    zoomedWH - pxWH,
                    zoomedWH - pxWH,
                    zLevel,
                ...
                t.addVertexWithUV(zoomedWH - pxWH, pxWH, zLevel, uvTexture.key + res.getULength(), uvTexture.value);
                t.addVertexWithUV(pxWH, pxWH, zLevel, uvTexture.key, uvTexture.value);
                t.draw();
                GL11.glColor4f(1F, 1F, 1F, 1F);
                TextureHelper.refreshTextureBindState();
                GL11.glEnable(GL11.GL_ALPHA_TEST);
    ...
    private void drawLinePart(double lx, double ly, double hx, double hy, double zLevel, float brightness) {
        ...
        t.addVertex(lx, ly, zLevel);
        t.addVertex(hx, hy, zLevel);
        t.draw();
    }
    ...
    private void drawResearchItemBackground(double zoomedWH, double xAdd, double yAdd, float zLevel) {
        ...
        Tessellator t = Tessellator.instance;
        t.startDrawingQuads();
        t.addVertexWithUV(renderOffsetX + xAdd, renderOffsetY + yAdd + zoomedWH, zLevel, 0, 1);
        t.addVertexWithUV(renderOffsetX + xAdd + zoomedWH, renderOffsetY + yAdd + zoomedWH, zLevel, 1, 1);
        t.addVertexWithUV(renderOffsetX + xAdd + zoomedWH, renderOffsetY + yAdd, zLevel, 1, 0);
        t.addVertexWithUV(renderOffsetX + xAdd, renderOffsetY + yAdd, zLevel, 0, 0);
        t.draw();
        GL11.glColor4f(1F, 1F, 1F, 1F);
    }

...
public class TESRAttunementAltar extends TileEntitySpecialRenderer {
    ...
    public void renderTileEntityAt(TileEntity te, double x, double y, double z, float partialTicks) {
        if (!(te instanceof TileAttunementAltar)) return;

        TileAttunementAltar tile = (TileAttunementAltar) te;
        renderTileAttunementAltar(tile, x, y, z, partialTicks);
    }

    private void renderTileAttunementAltar(TileAttunementAltar te, double x, double y, double z, float partialTicks) {
        ...
        for (int i = 1; i < 9; i++) {
            ...
            float xOffset = WrapMathHelper.cos(normalized);
            float zOffset = WrapMathHelper.sin(normalized);

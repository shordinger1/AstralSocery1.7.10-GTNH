...
public class RenderAstralSkybox extends IRenderHandler {
    ...
    private void setupStarVertices() {
        ...
        for (int i = 0; i < starLists.length; i++) {
            ...
            setupStars(tessellator, vRand, starAmountMap[i], starSizeMap[i]);
            tessellator.draw();
            GL11.glEndList();
        }
    }
    ...
    private void setupStars(Tessellator tessellator, Random random, int amount, double multiplier) {
        for (int i = 0; i < amount; ++i) { // Amount of stars.
            ...
            if (d4 < 1.0D && d4 > 0.01D) {
                ...
                for (int j = 0; j < 4; ++j) {
                    ...
                    tessellator.addVertexWithUV(d5 + d25, d6 + d23, d7 + d26, ((j + 1) & 2) >> 1, ((j + 2) & 2) >> 1);
                }
            }
        }
    }
    ...
    private void setupBackground(boolean invert) {
        ...
        for (int k = -384; k <= 384; k += 64) {
            for (int l = -384; l <= 384; l += 64) {
                ...
                tessellator.addVertex(p, 16, l);
                tessellator.addVertex(px, 16, l);
                tessellator.addVertex(px, 16, l + 64);
                tessellator.addVertex(p, 16, l + 64);
            }
        }
    }
    ...
    private void renderSky(float partialTicks) {
        ...
        // 1.7.10: getPositionEyes() doesn't exist, calculate manually
        double absPlayerHorizon = (Minecraft.getMinecraft().thePlayer.posY
            + Minecraft.getMinecraft().thePlayer.getEyeHeight()) - Minecraft.getMinecraft().theWorld.getHorizon();
    ...
    private void renderDefaultCelestials(float partialTicks) {
        ...
        if (handle != null && handle.getCurrentlyActiveEvent() == CelestialEvent.SOLAR_ECLIPSE) {
            renderSolarEclipseSun(handle, partialTicks, alphaSubRain);
        } else {
            renderSun();
        }
         ...
         * // 1.7.10: WorldRenderer incompatible
         * //WorldRenderer vb = tes.getWorldRenderer();
         ...
         * TEX_DEBUG.bind();
         * vb.begin(7, // TODO: DefaultVertexFormats - needs 1.7.10 equivalent
         * VertexFormat.POSITION_TEX_NORMAL);
         * for (int j = 0; j < 4; ++j) {
    ...
    private void renderSolarEclipseSun(WorldSkyHandler handle, float partialTicks, float alphaSubRain) {
        GL11.glColor4f(1.0F, 1.0F, 1.0F, alphaSubRain);
    ...
    private void renderSunsetToBackground(float[] sunsetColors, float partialTicks) {
        ...
        // 1.7.10: getCelestialAngleRadians() doesn't exist, use getCelestialAngle() and convert
        float celestialAngle = Minecraft.getMinecraft().theWorld.getCelestialAngle(partialTicks);
        GL11.glRotatef(
            WrapMathHelper.sin((float) Math.toRadians(celestialAngle * 360.0F)) < 0.0F ? 180.0F : 0.0F,
            0.0F,
            0.0F,
            1.0F);
        ...
        // 1.7.10: setColorRGBA_I takes 2 parameters (packed RGB, alpha), not 4
        int rgb1 = ((int) (f6 * 255) << 16) | ((int) (f7 * 255) << 8) | (int) (f8 * 255);
        tessellator.setColorRGBA_I(rgb1, (int) (sunsetColors[3] * 255));
        ...
        for (int l = 0; l <= 16; ++l) {
            ...
            // 1.7.10: setColorRGBA_I takes 2 parameters (packed RGB, alpha), not 4
            int rgb2 = ((int) (sunsetColors[0] * 255) << 16) | ((int) (sunsetColors[1] * 255) << 8)
                | (int) (sunsetColors[2] * 255);
            tessellator.setColorRGBA_I(rgb2, 0);
            tessellator

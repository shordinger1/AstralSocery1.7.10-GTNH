...
public class PersistentDataManager {
    ...
    public <T extends CachedPersistentData> T getData(PersistentKey key) {
        // 1.7.10: getOrDefault not available, use containsKey check
        if (this.cachedPersistentData.containsKey(key)) {
            return (T) this.cachedPersistentData.get(key);
        }
        return (T) key.provideEmptyInstance();
    }

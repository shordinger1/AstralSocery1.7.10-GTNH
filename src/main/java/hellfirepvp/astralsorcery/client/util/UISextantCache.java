...
public class UISextantCache {
    ...
    public static void addTarget(SextantFinder.TargetObject to, BlockPos pos, int dim) {
        ...
        BlockPos at = new BlockPos(Minecraft.getMinecraft().thePlayer);
        ChunkPos chAt = new ChunkPos(at);

        Tuple<ChunkPos, Integer> key = new Tuple<>(chAt, dim);
        List<CachedSextantResult> cache = sextantCache.get(key);
        if (cache == null) {
            cache = new LinkedList<>();
            sextantCache.put(key, cache);
        }
        Iterator<CachedSextantResult> it = cache.iterator();
        while (it.hasNext()) {
            if (it.next().target.equals(to)) {
                it.remove();
            }
        }

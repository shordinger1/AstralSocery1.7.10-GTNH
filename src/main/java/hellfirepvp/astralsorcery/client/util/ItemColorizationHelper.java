...
import hellfirepvp.astralsorcery.common.migration.IResourceManager;
import hellfirepvp.astralsorcery.common.migration.IResourceManagerReloadListener;
...
public class ItemColorizationHelper implements IResourceManagerReloadListener {
    ...
    private Color getDominantColorFromStack(ItemStack stack) {
        ...
        try {
            ...
            r = WrapMathHelper.clamp(r, 0, 255);
            g = WrapMathHelper.clamp(g, 0, 255);
            b = WrapMathHelper.clamp(b, 0, 255);
    ...
    private int getOverlayColor(ItemStack stack) {
        if ((stack == null || stack.stackSize <= 0)) return -1;
        if (stack.getItem() instanceof ItemBlock) {
            Block state = ItemUtils.createBlockState(stack);
            if (state == null) return -1;
            // 1.7.10: Call colorMultiplier directly on Block
            return state.colorMultiplier(null, 0, 0, 0);
        } else {
            // 1.7.10: Call getColorFromItemStack directly on Item

...
public class RenderConstellation {
    ...
    public static void renderConstellationIntoWorldFlat(IConstellation c, Color rC, Vector3 offsetPos, double scale,
        ...
        for (StarConnection sc : c.getStarConnections()) {
            ...
            int rgb = ((int) (fRed * 255) << 16) | ((int) (fGreen * 255) << 8) | (int) (fBlue * 255);
            tes.setColorRGBA_I(rgb, (int) (fAlpha * 255));
            ...
            Vector3 pos = offsetRender.clone()
                ...
                .add(
                    dirV.clone()
                        .multiply(1));
            tes.addVertexWithUV(pos.getX(), pos.getY(), pos.getZ(), 1, 0);
            pos = offsetRender.clone()
                ...
                .add(
                    dirV.clone()
                        .multiply(1));
            tes.addVertexWithUV(pos.getX(), pos.getY(), pos.getZ(), 0, 0);
            pos = offsetRender.clone()
                .add(
                    dirU.clone()
                        .multiply(1))
                .add(
                    dirV.clone()
                        .multiply(0));
            tes.addVertexWithUV(pos.getX(), pos.getY(), pos.getZ(), 0, 1);
            pos = offsetRender.clone()
                .add(
                    dirU.clone()
                        .multiply(0))
                .add(
                    dirV.clone()
                        .multiply(0));
            tes.addVertexWithUV(pos.getX(), pos.getY(), pos.getZ(), 1, 1);
                ...
                for (int i = 0; i < 4; i++) {
                    Vector3 pos = offset00.clone()
                        ...
                        .add(
                            vecCV.clone()
                                .multiply(((i + 2) & 2) >> 1));
                    tes.addVertexWithUV(pos.getX(), pos.getY(), pos.getZ(), ((i + 2) & 2) >> 1, ((i + 3) & 2) >> 1);
                }
                tes.draw();
            }
        }
    ...
    public static float flickerSin(long wtime, float partialTicks, double divisor, float div, float move) {
        ...
        float sin = WrapMathHelper.sin((float) rad);
        return (sin / div) + move;
    }

...
public class ClientConnectionEventHandler {
    ...
    public void onJoin(FMLNetworkEvent.ClientConnectedToServerEvent event) {
        AstralSorcery.proxy.scheduleClientside(() -> {
            ...
            if (nm.isLocalChannel()) {
                ...
                int id = addr.indexOf('\\');
                if (id != -1) {
                    addr = addr.substring(0, WrapMathHelper.clamp(id, 1, addr.length()));
                }
            }
    ...
    private String sanitizeFileName(String addr) {
        ...
        // ILLEGAL_FILE_CHARACTERS from 1.12.2 ChatAllowedCharacters
        final char[] ILLEGAL_FILE_CHARACTERS = { '/', '\n', '\r', '\t', '\u0000', '\f', '`', '?', '*', '\\', '<', '>',
            '|', '"', ':' };

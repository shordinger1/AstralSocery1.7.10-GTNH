...
public class EffectLightning extends EntityComplexFX {
    ...
    public static EffectLightning buildAndRegisterLightning(Vector3 source, Vector3 destination) {
        ...
        if (dstLength > optimalLightningLength) {
            perc = WrapMathHelper.sqrt((float) (dstLength / optimalLightningLength));
    ...
    public static EffectLightning buildLightning(long seed, Vector3 source, Vector3 destination,
        ...
        int iterations = WrapMathHelper.floor(Math.round(Math.sqrt(l)));
    ...
    private void renderF(float partialTicks, Tessellator vb) {
        bufRenderDepth = Math.min(1F, (age + partialTicks) / (buildSpeed * 20F));
        renderRec(this.root, vb);
    }

    private void renderRec(LightningVertex root, Tessellator vb) {
    ...
    private void drawLine(Vector3 from, Vector3 to, Tessellator vb) {
    ...
    private void renderCurrentTextureAroundAxis(Vector3 from, Vector3 to, double angle, double size, Tessellator buf) {
        ...
        buf.addVertexWithUV(vec.getX(), vec.getY(), vec.getZ(), 1, 1);

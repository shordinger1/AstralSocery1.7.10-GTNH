    ...
    private void renderCurrentTextureAroundAxis(double angle) {
        ...
        Vector3 vec = from.clone()
            .add(
                perpFrom.clone()
                    .multiply(-1));
        tes.addVertexWithUV(vec.getX(), vec.getY(), vec.getZ(), u, v + vHeight);
        vec = from.clone()
            .add(perpFrom);
        tes.addVertexWithUV(vec.getX(), vec.getY(), vec.getZ(), u + uWidth, v + vHeight);
        vec = to.clone()
            .add(perpTo);
        tes.addVertexWithUV(vec.getX(), vec.getY(), vec.getZ(), u + uWidth, v);
        vec = to.clone()
            .add(
                perpTo.clone()
                    .multiply(-1));

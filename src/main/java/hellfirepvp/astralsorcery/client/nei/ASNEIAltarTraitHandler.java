 ...
 *
 * ASNEIAltarTraitHandler - NEI recipe handler for Trait Altar
 ******************************************************************************/
...
/**
 * NEI Recipe Handler for Trait Altar
 */
public class ASNEIAltarTraitHandler extends TemplateRecipeHandler {
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId.equals(getOverlayIdentifier())) {
            List<ASAltarRecipe> recipes = AltarRecipeRegistry.getRecipesForLevel(TileAltar.AltarLevel.TRAIT_CRAFT);
            System.out.println("[ASNEI Trait] Loading " + recipes.size() + " recipes");
    ...
    public void loadCraftingRecipes(ItemStack result) {
        ...
        for (ASAltarRecipe recipe : recipes) {
            if (recipe.getAltarLevel() == TileAltar.AltarLevel.TRAIT_CRAFT) {
                arecipes.add(new CachedAltarRecipe(recipe));
            }
        }
    }
    ...
    public TemplateRecipeHandler newInstance() {
        return new ASNEIAltarTraitHandler();
    }
    ...
    public class CachedAltarRecipe extends CachedRecipe {
        ...
        public CachedAltarRecipe(ASAltarRecipe recipe) {
            ...
            for (int i = 0; i < recipeInputs.length && i < TRAIT_LAYOUT.length; i++) {
                if (recipeInputs[i] != null) {
                    int[] pos = TRAIT_LAYOUT[i];

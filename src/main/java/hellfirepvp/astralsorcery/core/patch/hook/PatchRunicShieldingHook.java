...
public class PatchRunicShieldingHook extends ClassPatch {
    ...
    public void patch(ClassNode cn) {
        ...
        AbstractInsnNode getStaticMaxCharge = findFirstInstructionAfter(mn, 0, new ClassPatch.CustomPredicate() {

            @Override
            public boolean test(AbstractInsnNode node) {
                return node.getOpcode() == Opcodes.GETSTATIC && node instanceof FieldInsnNode
                    && ((FieldInsnNode) node).name.equals("lastMaxCharge");
            }
        }).getPrevious();

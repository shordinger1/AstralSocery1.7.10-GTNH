...
public class BlockTransmutationHandler implements StarlightNetworkRegistry.IStarlightBlockHandler {
    ...
    @Override
    @Deprecated
    public boolean isApplicable(World world, BlockPos pos, Block state) {
        return false;
    }

    @Override
    public boolean isApplicable(World world, BlockPos pos, Block state, @Nullable IWeakConstellation starlightType) {
    ...
    public void receiveStarlight(World world, Random rand, BlockPos pos, @Nullable IWeakConstellation starlightType,
        ...
        if (LightOreTransmutations.searchForTransmutation(world.getBlock(pos.getX(), pos.getY(), pos.getZ()))
        ...
        if (node.accCharge >= node.runningTransmutation.getCost()) {
            if (!world.setBlock(pos.getX(), pos.getY(), pos.getZ(), node.runningTransmutation.getOutput(), 0, 3)) {
                node.accCharge -= 1000;
            } else {
                runningTransmutations.remove(pos);
            }

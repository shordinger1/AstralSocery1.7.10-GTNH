    ...
    public static boolean compare(@Nonnull ItemStack thisStack, @Nonnull ItemStack sampleCompare, Clause... clauses) {
        ...
        if (lClauses.contains(Clause.ITEM)) {
            if ((thisStack == null || thisStack.stackSize <= 0)
                && !(sampleCompare == null || sampleCompare.stackSize <= 0)) {
                return false;
            }
            if (!(thisStack == null || thisStack.stackSize <= 0) && !thisStack.getItem()
        ...
        if (lClauses.contains(Clause.AMOUNT_EXACT)) {
            if (thisStack.stackSize != sampleCompare.stackSize) {
                return false;
            }
        ...
        if (lClauses.contains(Clause.CAPABILITIES_COMPATIBLE)) {
            // In 1.7.10, the Forge Capability system doesn't exist.
            // Skip this check - always return true for capability compatibility.
            // Capabilities were introduced in Minecraft 1.8+.
        }

...
public class CrystalCalculations {
    ...
    public static int getChannelingCapacity(CrystalProperties properties) {
        ...
        return Math.max(10, WrapMathHelper.floor(purity * 20));
    }
    ...
    public static float getPerfection(CrystalProperties properties, int maxSize) {
        float purity = WrapMathHelper.sqrt((float) (((float) properties.getPurity()) / 100D));
        float cutting = WrapMathHelper.sqrt((float) (((float) properties.getCollectiveCapability()) / 100D));
        float size = WrapMathHelper.sqrt((((float) properties.getSize()) / maxSize));

...
public class TileRitualPedestal extends TileReceiverBase
    ...
    public void updateEntity() {
        ...
        if (!getWorld().isRemote) {
            ...
            if (dirty || !(clientCatalystCache == null || clientCatalystCache.stackSize <= 0)) {
        ...
        if (getWorld().isRemote && working) {
            ...
            if (!(clientCatalystCache == null || clientCatalystCache.stackSize <= 0)
                && clientCatalystCache.getItem() instanceof ItemTunedCrystalBase) {
    ...
    public ItemStack placeCrystalIntoPedestal(ItemStack crystal) {
        this.clientCatalystCache = ItemUtils.copyStackWithSize(crystal, Math.min(crystal.stackSize, 1));
        markForUpdate();

        TransmissionReceiverRitualPedestal recNode = getUpdateCache();
        if (recNode != null) {
            markForUpdate();
            recNode.setChannelingCrystal(crystal, worldObj);
        }
        return ItemUtils.copyStackWithSize(crystal, Math.max(0, crystal.stackSize - 1));
    }
    ...
    public IWeakConstellation getRitualConstellation() {
        ...
        if (!(crystal == null || crystal.stackSize <= 0) && crystal.getItem() instanceof ItemTunedCrystalBase) {
     ...
     * if(!(in == null || in.stackSize <= 0) && in.getItem() instanceof ItemTunedCrystalBase) {
    ...
    public void readCustomNBT(NBTTagCompound compound) {
        ...
        if (compound.hasKey("ownerMost")) {
            this.ownerUUID = NBTHelper.getUniqueId(compound, "owner");
        } else {
    ...
    public void writeCustomNBT(NBTTagCompound compound) {
        ...
        if (ownerUUID != null) {
            NBTHelper.setUniqueId(compound, "owner", ownerUUID);
        }
    ...
    public static class TransmissionReceiverRitualPedestal extends SimpleTransmissionReceiver {
        ...
        private void doMainEffect(World world, ConstellationEffect ce, @Nullable IMinorConstellation trait) {
            ...
            int executeTimes = WrapMathHelper.floor(collectionChannelBuffer / maxDrain);

            int freeCap = WrapMathHelper.floor(
            ...
            executeTimes = WrapMathHelper.floor(executeTimes * prop.getEffectAmplifier());
        ...
        public void writeToNBT(NBTTagCompound compound) {
            ...
            if (ce != null) {
                NBTHelper.setAsSubTag(compound, "effect", new Consumer<NBTTagCompound>() {

                    @Override
                    public void accept(NBTTagCompound nbt) {
                        ce.writeToNBT(nbt);
                    }
                });
            }
        ...
        public void setChannelingCrystal(ItemStack crystal, World world) {
            this.crystal = ItemUtils.copyStackWithSize(crystal, Math.min(crystal.stackSize, 1));
            markDirty(world);
        }

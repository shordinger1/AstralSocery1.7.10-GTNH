...
public class TileBore extends TileInventoryBase implements IMultiblockDependantTile, ILiquidStarlightPowered {
    ...
    private void playBoreVortexEffect() {
        ...
        List<EntityLivingBase> entities = worldObj.getEntitiesWithinAABB(EntityLivingBase.class, boxVortex);
        ...
        List<EntityLivingBase> draws = worldObj.getEntitiesWithinAABB(EntityLivingBase.class, drawBox);
        ...
        for (EntityLivingBase e : draws) {
            ...
            EntityUtils.applyVortexMotion(new Function<Void, Vector3>() {
                ...
                @Override
                public Vector3 apply(Void aVoid) {
                    return Vector3.atEntityCorner(e);
                }
            }, new Function<Vector3, Object>() {

                @Override
                public Object apply(Vector3 v) {
                    if (e instanceof EntityDragon) {
                        e.posX += v.getX();
                        e.posY += v.getY();
                        e.posZ += v.getZ();
        ...
        if (productionTimeout <= 0) {
            ...
            if (entry != null) {
                ...
                if (entry.isValid() && actMbDrain > 0) {
                    ...
                    Iterator<TileChalice> it1 = out.iterator();
                    while (it1.hasNext()) {
                        TileChalice t = it1.next();
                        if (t.getPos()
                            .equals(getPos().up())) {
                            it1.remove();
                        }
                    ...
                    drained = new FluidStack(FluidRegistry.WATER, mbDrain);
                    List<TileChalice> out = LiquidStarlightChaliceHandler.findNearbyChalicesWithSpaceFor(this, drained);
                    Iterator<TileChalice> it2 = out.iterator();
                    while (it2.hasNext()) {
                        TileChalice t = it2.next();
                        if (t.getPos()
                            .equals(getPos().up())) {
                            it2.remove();
                        }
    ...
    private void playLowVortex() {
        ...
        for (int i = 0; i < 3; i++) {
            Vector3 particlePos = new Vector3(
                getPos().getX() - 4 + rand.nextFloat() * 9,
                getPos().getY() - 2 + this.vortexOffset + rand.nextFloat() * 9,
                getPos().getZ() - 4 + rand.nextFloat() * 9);
            Vector3 dir = particlePos.clone()
                .subtract(getPos().getX() + 0.5, getPos().getY() + 0.5 + this.vortexOffset, getPos().getZ() + 0.5)
                .normalize()
                .divide(-30);
    ...
    private void updateNoisePlane() {
        ...
        if ((spr == null || spr.canRemove() || spr.isRemoved()) && this.operationTicks > 0) {
            spr = EntityFXFacingSprite.fromSpriteSheet(
                ...
                2F,
                2);
            spr.setRefreshFunc(new EntityComplexFX.RefreshFunction() {
    ...
    private void vortexExplosion() {
        for (int i = 0; i < 140; i++) {
            Vector3 particlePos = new Vector3(
                getPos().getX() + 0.5 - 0.1F + rand.nextFloat() * 0.2,
                getPos().getY() + 0.5 + this.vortexOffset - 0.1F + rand.nextFloat() * 0.2,
                getPos().getZ() + 0.5 - 0.1F + rand.nextFloat() * 0.2);
            Vector3 dir = new Vector3(
    ...
    private void playVortexCore(float prepChance) {
        ...
        for (int i = 0; i < 15; i++) {
            Vector3 particlePos = new Vector3(
                getPos().getX() + 0.5 - 0.1F + rand.nextFloat() * 0.2,
                getPos().getY() + yOffset - 0.1F + rand.nextFloat() * 0.2,
                getPos().getZ() + 0.5 - 0.1F + rand.nextFloat() * 0.2);
    ...
    private void playInnerVortex(double chance) {
        for (int i = 0; i < 12; i++) {
            if (rand.nextFloat() < chance) {
                Vector3 particlePos = new Vector3(
                    getPos().getX() - 0.4 + rand.nextFloat() * 1.8,
                    getPos().getY() - rand.nextFloat() * 3,
                    getPos().getZ() - 0.4 + rand.nextFloat() * 1.8);
                Vector3 dir = particlePos.clone()
                    .subtract(getPos().getX() + 0.5, getPos().getY() + 0.5, getPos().getZ() + 0.5)
                    .normalize()
                    .divide(-30);
                EntityFXFacingParticle p = EffectHelper
                    .genericFlareParticle(particlePos.getX(), particlePos.getY(), particlePos.getZ());
    ...
    private void playCoreParticles(float chance) {
        for (int i = 0; i < 20; i++) {
            if (rand.nextFloat() < chance) {
                Vector3 particlePos = new Vector3(
                    getPos().getX() - 1 + rand.nextFloat() * 3,
                    getPos().getY() - 1.5 + rand.nextFloat() * 2,
                    getPos().getZ() - 1 + rand.nextFloat() * 3);
                Vector3 dir = particlePos.clone()
                    .subtract(getPos().getX() + 0.5, getPos().getY() - 0.5, getPos().getZ() + 0.5)
                    .normalize()
                    .divide(-30);
    ...
    private void playVortex(float chance) {
        for (int i = 0; i < 20; i++) {
            if (rand.nextFloat() < chance) {
                Vector3 particlePos = new Vector3(
                    getPos().getX() - 3 + rand.nextFloat() * 7,
                    getPos().getY() + rand.nextFloat(),
                    getPos().getZ() - 3 + rand.nextFloat() * 7);
                Vector3 dir = particlePos.clone()
                    .subtract(getPos().getX() + 0.5, getPos().getY() + 0.5, getPos().getZ() + 0.5)
                    .normalize()
                    .divide(-30);
    ...
    private void updateMultiblockState() {
        ...
        if (found != this.hasMultiblock) {
            LogCategory.STRUCTURE_MATCH.info(new Provider<String>() {

                @Override
                public String provide() {
                    return "Structure match updated: " + TileBore.this.getClass()
                        .getName()
                        + " at "
                        + TileBore.this.getPos()
                        + " ("
                        + TileBore.this.hasMultiblock
                        + " -> "
                        + found
                        + ")";
                }
    ...
    private TextureSpritePlane updateBoreSprite() {
        ...
        if ((spr == null || spr.canRemove() || spr.isRemoved()) && this.operationTicks > 0) {
            ...
            spr.setRefreshFunc(new EntityComplexFX.RefreshFunction() {
                ...
                @Override
                public boolean shouldRefresh() {
                    if (isInvalid() || getCurrentBoreType() == null || TileBore.this.operationTicks <= 0) {
                        return false;
                    }
                    if (TileBore.this.worldObj.provider == null || Minecraft.getMinecraft().theWorld == null
                        || Minecraft.getMinecraft().theWorld.provider == null) {
                        return false;
                    }
                    return TileBore.this.worldObj.provider.dimensionId
    ...
    public void readCustomNBT(NBTTagCompound compound) {
        ...
        if (!tank.canAccess(EnumFacing.UP)) {

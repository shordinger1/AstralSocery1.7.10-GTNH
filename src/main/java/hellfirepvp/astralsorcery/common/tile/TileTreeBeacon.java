...
public class TileTreeBeacon extends TileReceiverBase implements IStructureAreaOfInfluence {
    ...
    private NonDuplicateCappedWeightedList<BlockPos> treePositions = new NonDuplicateCappedWeightedList<>(
        WrapMathHelper.floor(ConfigEntryTreeBeacon.maxCount));
    ...
    public void updateEntity() {
        ...
        if (getWorld().isRemote) {
            ...
            if (treePositions.getCap() != ConfigEntryTreeBeacon.maxCount) {
                treePositions.setCap(WrapMathHelper.floor(ConfigEntryTreeBeacon.maxCount)); // Post-update
            }
            ...
            int runs = WrapMathHelper.ceil(starlightCharge * 1.4D);
            starlightCharge = 0D;
        ...
        if (rand.nextInt(ConfigEntryTreeBeacon.dropsChance) == 0) {
            if (MiscUtils.canEntityTickAt(world, pos)) {
                ...
                List<ItemStack> drops = b.getDrops(
                    ...
                    treeBlockPos.getY(),
                    treeBlockPos.getZ(),
                    world.getBlockMetadata(treeBlockPos.getX(), treeBlockPos.getY(), treeBlockPos.getZ()),
                    2);
    ...
    private void playEffects() {
        ...
        if (getPlacedBy() != null) {
            ...
            for (PatreonEffectHelper.PatreonEffect p : PatreonEffectHelper
                .getPatreonEffects(Side.CLIENT, getPlacedBy())) {
                if (p instanceof PtEffectTreeBeacon) {
                    effect = p;
                    break;
                }
            }
        ...
        if (tft != null && tft.getReference() != null) {
            ...
            if (ttb != null) {
                ...
                if (ttb.getPlacedBy() != null) {
                    PatreonEffectHelper.PatreonEffect effect = null;
                    for (PatreonEffectHelper.PatreonEffect p : PatreonEffectHelper
                        .getPatreonEffects(Side.CLIENT, ttb.getPlacedBy())) {
                        if (p instanceof PtEffectTreeBeacon) {
                            effect = p;
                            break;
                        }

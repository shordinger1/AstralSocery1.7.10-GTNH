...
public class AttuneCrystalTrigger extends ListenerCriterionTrigger<ConstellationInstance> {
    ...
    public void trigger(EntityPlayerMP player, IConstellation attuned) {
        Listeners<ConstellationInstance> listeners = this.listeners.get(AdvancementManager.getAdvancements(player));
        if (listeners != null) {
            listeners.trigger(new ListenerCriterionTrigger.CustomPredicate() {

                @Override
                public boolean test(ICriterionInstance instance) {
                    return ((ConstellationInstance) instance).test(attuned);
                }
            });
        }
    }

...
public class GrindstoneRecipeRegistry {
    ...
    public static Collection<GrindstoneRecipe> getValidRecipes() {
        List<GrindstoneRecipe> result = new ArrayList<>();
        for (GrindstoneRecipe recipe : recipes) {
            if (recipe.isValid()
                && !(recipe instanceof CrystalToolSharpeningRecipe || recipe instanceof CrystalSharpeningRecipe
                    || recipe instanceof SwordSharpeningRecipe)) {
                result.add(recipe);
            }
        }
        return result;

...
public class CelestialGatewaySystem {
    ...
    public void onServerStart() {
        ...
        for (Integer id : worlds) {
            ...
            WorldServer world = server.worldServerForDimension(id); // 1.7.10: Use worldServerForDimension instead of
                                                                    // getWorld
            ...
            if (world.getChunkProvider()
                ...
                && ForgeChunkManager.getPersistentChunksFor(world)
                    .size() == 0
                && !world.provider.canRespawnHere()) { // 1.7.10: No getDimensionType() method
    ...
    public void onWorldInit(WorldEvent.Load event) {
        ...
        if (world.isRemote) return; // 1.7.10: Use world.isRemote instead of getWorld().isRemote
    ...
    private void forceLoad(int dim) {
        WorldServer serv = DimensionManager.getWorld(dim); // 1.7.10: DimensionManager.getWorld() may return null for
                                                           // unloaded dims
        if (serv == null) {
    ...
    public static class GatewayWorldFilter {
        ...
        private void loadCache() {
            try {
                ...
                // 1.7.10: Need to iterate through the NBTTagList manually
                // Use reflection to access the private tagList field
                ...
                for (int i = 0; i < tagList.size(); i++) {
                    ...
                    if (nbtbase instanceof NBTTagInt) {
                        Integer id = ((NBTTagInt) nbtbase).func_150287_d();

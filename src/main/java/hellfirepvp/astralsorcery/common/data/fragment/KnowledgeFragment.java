...
public abstract class KnowledgeFragment {
    ...
    public boolean isFullyPresent() {
        return StatCollector.canTranslate(getUnlocalizedName()) && StatCollector.canTranslate(getUnlocalizedBookmark())
            && StatCollector.canTranslate(getUnlocalizedPage())
            && (unlocPrefix == null || unlocPrefix.length() <= 0 || StatCollector.canTranslate(this.unlocPrefix));
    }
    ...
    public String getLocalizedIndexName() {
        return unlocPrefix == null || unlocPrefix.length() <= 0 ? this.getLocalizedName()

...
public class DataPatreonFlares extends AbstractData {
    ...
    public Collection<Collection<PatreonPartialEntity>> getEntities(Side side) {
        if (side == Side.CLIENT) {
            List<Collection<PatreonPartialEntity>> result = new ArrayList<>();
            for (Map<PatreonEffectHelper.PatreonEffect, PatreonPartialEntity> map : patreonFlaresClient.values()) {
                result.add(map.values());
            }
            return result;
        } else {
            List<Collection<PatreonPartialEntity>> result = new ArrayList<>();
            for (Map<PatreonEffectHelper.PatreonEffect, PatreonPartialEntity> map : patreonFlaresServer.values()) {
                result.add(map.values());
            }
            return result;
        }
    ...
    public PatreonPartialEntity createEntity(EntityPlayer player, PatreonEffectHelper.PatreonEffect value) {
        ...
        if (!patreonFlaresServer.containsKey(owner)) {
            patreonFlaresServer.put(owner, new HashMap<>());
        }
        patreonFlaresServer.get(owner)
            .put(value, entity);
    ...
    public void writeAllDataToPacket(NBTTagCompound compound) {
        ...
        for (Map.Entry<UUID, Map<PatreonEffectHelper.PatreonEffect, PatreonPartialEntity>> flares : this.patreonFlaresServer
            ...
            NBTHelper.setUniqueId(tag, "owner", flares.getKey());
    ...
    public void writeToPacket(NBTTagCompound compound) {
        ...
        for (UUID added : this.flareAdditions) {
            ...
            NBTHelper.setUniqueId(tag, "owner", added);
    ...
    public void handleIncomingData(AbstractData serverData) {
        ...
        for (int i = 0; i < add.tagCount(); i++) {
            ...
            UUID owner = NBTHelper.getUniqueId(cmp, "owner");
            ...
            for (int j = 0; j < flareList.tagCount(); j++) {
                ...
                UUID effectId = NBTHelper.getUniqueId(containerTag, "id");
                NBTTagCompound flareTag = containerTag.getCompoundTag("data");

                PatreonEffectHelper.PatreonEffect pe = null;
                for (PatreonEffectHelper.PatreonEffect p : PatreonEffectHelper.getPatreonEffects(Side.SERVER, owner)) {
                    if (p.getId()
                ...
                if (pe != null) {
                    if (!this.patreonFlaresClient.containsKey(owner)) {
                        this.patreonFlaresClient.put(owner, new HashMap<>());
                    }

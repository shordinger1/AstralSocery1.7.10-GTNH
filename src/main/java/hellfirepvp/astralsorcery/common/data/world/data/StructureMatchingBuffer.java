...
public class StructureMatchingBuffer extends CachedWorldData {
    ...
    public <T extends StructureMatcher> ChangeSubscriber<T> observeArea(BlockPos requester, T matcher) {
        ...
        for (ChunkPos pos : subscriber.getObservableChunks()) {
            if (!this.subscribers.containsKey(pos)) {
                this.subscribers.put(pos, Lists.newArrayList());
            }
            this.subscribers.get(pos)
                .add(subscriber);
        }
    ...
    public boolean removeSubscriber(BlockPos pos) {
        if (requestSubscribers.remove(pos) != null) {
            ...
            if (!subscribers.containsKey(chunk)) {
                subscribers.put(chunk, Lists.newArrayList());
            }
            List<ChangeSubscriber<?>> chunkSubscribers = subscribers.get(chunk);
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        for (int i = 0; i < subscriberList.tagCount(); i++) {
            ...
            for (ChunkPos chPos : subscriber.getObservableChunks()) {
                if (!this.subscribers.containsKey(chPos)) {
                    this.subscribers.put(chPos, Lists.newArrayList());
                }
                this.subscribers.get(chPos)
                    .add(subscriber);
            }
        }
    }
    ...
    public void writeToNBT(NBTTagCompound compound) {
        ...
        for (ChangeSubscriber<?> sub : this.requestSubscribers.values()) {
            ...
            // 1.7.10: Manual NBT writing instead of method reference
            NBTTagCompound matchData = new NBTTagCompound();
            sub.writeToNBT(matchData);
            subscriber.setTag("matchData", matchData);

            ...
            for (StorageNetwork.CoreArea core : network.getCores()) {
                ...
                for (int chX = from.x; chX <= to.x; chX++) {
                    for (int chZ = from.z; chZ <= to.z; chZ++) {
                        if (!this.availableNetworks.containsKey(new ChunkPos(chX, chZ))) {
                            this.availableNetworks.put(new ChunkPos(chX, chZ), Lists.newArrayList());
                        }
                        this.availableNetworks.get(new ChunkPos(chX, chZ))
                            .add(network);
                    }
                }
            }

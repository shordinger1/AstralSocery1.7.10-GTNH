...
public class ActiveStarMap {
    ...
    private static void shiftDistribution(ActiveStarMap asm) {
        ...
        for (Map.Entry<IConstellation, Float> entry : asm.starProportions.entrySet()) {
            ...
            perc = WrapMathHelper.clamp(perc, 0F, 1F);
            if (perc >= 0.75F) {
                perc = (perc * 3F) - 2.25F;
                perc *= perc;
                perc = -perc + 1;
        ...
        for (Map.Entry<IConstellation, Float> entry : asm.starProportions.entrySet()) {
            float perc = entry.getValue();
            out.put(entry.getKey(), WrapMathHelper.sqrt(perc));
        }
        asm.starProportions = out;
    }
        ...
        lblEnchantments: for (Map.Entry<IConstellation, List<ConstellationMapEffectRegistry.EnchantmentMapEffect>> entry : effectMap
            ...
            for (ConstellationMapEffectRegistry.EnchantmentMapEffect effect : entry.getValue()) {
                ...
                if (!effect.ignoreCompaibility) {
                    for (Integer existingId : enchantments.keySet()) {
                        Enchantment existing = Enchantment.enchantmentsList[existingId];
                        if (existing != null && !existing.canApplyTogether(effect.ench)) {
                ...
                float p = WrapMathHelper.clamp(perc, 0F, 1F);
    ...
    public void tryApplyPotionEffects(ItemStack stack) {
            ...
            if (me != null) {
                for (ConstellationMapEffectRegistry.PotionMapEffect effect : me.potionEffects) {
                    ...
                    if ((eff = MiscUtils.iterativeSearch(
                        applicableEffects,
                        (pe) -> Potion.potionTypes[pe.getPotionID()].equals(effect.potion))) != null) {
        ...
        stack.setStackDisplayName("potion.as.crafted.name");
        Collections.shuffle(applicableEffects);

        // 1.7.10: Write potion effects to NBT manually
        NBTTagCompound nbt = NBTHelper.getData(stack);
        NBTTagList effects = new NBTTagList();
        for (PotionEffect effect : applicableEffects) {
            NBTTagCompound effectNbt = new NBTTagCompound();
            effectNbt.setByte("Id", (byte) effect.getPotionID());
            effectNbt.setByte("Amplifier", (byte) effect.getAmplifier());
            effectNbt.setInteger("Duration", effect.getDuration());
            effectNbt.setBoolean("Ambient", effect.getIsAmbient());
            effects.appendTag(effectNbt);
        }
        nbt.setTag("CustomPotionEffects", effects);
    }

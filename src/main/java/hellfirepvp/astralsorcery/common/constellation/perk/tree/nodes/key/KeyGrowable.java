...
public class KeyGrowable extends KeyPerk implements IPlayerTickPerk {
    ...
    protected void applyEffectMultiplier(double multiplier) {
        ...
        this.chanceToBonemeal = WrapMathHelper.ceil(this.chanceToBonemeal * multiplier);
        this.radius = WrapMathHelper.ceil(this.radius * multiplier);
    }
    ...
    public void onPlayerTick(EntityPlayer player, Side side) {
        ...
        int chance = Math.max(WrapMathHelper.floor(cChance), 1);
        if (rand.nextInt(chance) == 0) {
            ...
            int rRadius = Math.max(WrapMathHelper.floor(fRadius), 1);
            ...
            if (plant != null) {
                 ...
                 * ((IGrowable) at).grow(w, rand, pos, at);
                 * pkt = new PktParticleEvent(PktParticleEvent.ParticleEventType.CE_CROP_INTERACT, pos);
                 * }
                 * } else
                 */
                // 1.7.10: BlockDirt doesn't have VARIANT property. Use metadata 0 for regular dirt
                if (at instanceof BlockDirt && w.getBlockMetadata(pos.getX(), pos.getY(), pos.getZ()) == 0) {
                    if (w.setBlock(pos.getX(), pos.getY(), pos.getZ(), Blocks.grass, 0, 3)) {
                        pkt = new PktParticleEvent(PktParticleEvent.ParticleEventType.CE_CROP_INTERACT, pos);
                    }
                }
            }

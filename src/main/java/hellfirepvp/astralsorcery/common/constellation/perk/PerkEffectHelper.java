...
public class PerkEffectHelper implements ITickHandler {
    ...
    public void onConnect(FMLNetworkEvent.ServerConnectionFromClientEvent event) {
        LogCategory.PERKS.info(
            () -> ((NetHandlerPlayServer) event.handler).playerEntity.getCommandSenderName() + " connected to server on side SERVER");
        AstralSorcery.proxy.scheduleDelayed(
    ...
    public void expRemoval(LivingDeathEvent event) {
        if (event.entityLiving instanceof EntityPlayer) {
            ...
            if (prog.isValid()) {
                long exp = (long) Math.floor(prog.getPerkExp());
                ...
                int remove = WrapMathHelper.floor(((float) (expNextLevel - expThisLevel)) * removePerDeath);
    ...
    private void handlePerkModification(EntityPlayer player, Side side, boolean remove) {
        ...
        if (progress.isValid()) {
            LogCategory.PERKS.info(
                () -> (remove ? "Remove" : "Apply") + " ALL perks for " + player.getCommandSenderName() + " on side " + side.name());
    ...
    private void handlePerkApplication(AbstractPerk perk, EntityPlayer player, Side side) {
        ...
        LogCategory.PERKS.info(
            () -> "Apply perk " + perk
                .getRegistryName() + " for player " + player.getCommandSenderName() + " on side " + side.name());
    ...
    private synchronized void batchApplyConverters(EntityPlayer player, Side side, Collection<PerkConverter> converters,
        ...
        if (prog.isValid()) {
            ...
            List<AbstractPerk> filteredPerks = new ArrayList<>();
            for (AbstractPerk perk : perks) {
                if (attributeMap.isPerkApplied(perk)) {
            ...
            if (onlyAdd == null || !prog.isPerkSealed(onlyAdd)) {
                for (PerkConverter c : converters) {
                    attributeMap.applyConverter(player, c);
                }
            }
    ...
    private synchronized void batchRemoveConverters(EntityPlayer player, Side side,
        ...
        if (prog.isValid()) {
            ...
            for (PerkConverter c : converters) {
                attributeMap.removeConverter(player, c);
            }
    ...
    public final boolean isCooldownActiveForPlayer(EntityPlayer player, AbstractPerk perk) {
        ...
        TimeoutListContainer<PlayerWrapperContainer, ResourceLocation> container = player.worldObj.isRemote
            ? perkCooldownsClient
            : perkCooldowns;
            ...
            if (perk instanceof ICooldownPerk) {
                LogCategory.PERKS.info(
                    () -> "Perk cooldown has finished on " + perk.getRegistryName()
                        + " for "
                        + plWrapper.player.getCommandSenderName()
                        + " on "
                        + (plWrapper.player.worldObj.isRemote ? "CLIENT" : "SERVER"));

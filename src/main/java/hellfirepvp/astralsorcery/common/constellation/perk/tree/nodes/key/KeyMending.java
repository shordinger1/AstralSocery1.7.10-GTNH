...
public class KeyMending extends KeyPerk implements IPlayerTickPerk {
    ...
    protected void applyEffectMultiplier(double multiplier) {
        ...
        this.chanceToRepair = WrapMathHelper.ceil(this.chanceToRepair * multiplier);
    }
        ...
        if (side == Side.SERVER) {
            float fChance = PerkAttributeHelper.getOrCreateMap(player, side)
                .modifyValue(
                    ...
                    chanceToRepair);
            int chance = Math.max(WrapMathHelper.floor(fChance), 1);
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (!(armor == null || armor.stackSize <= 0) && armor.isItemStackDamageable()
                    && armor.isItemDamaged()) {

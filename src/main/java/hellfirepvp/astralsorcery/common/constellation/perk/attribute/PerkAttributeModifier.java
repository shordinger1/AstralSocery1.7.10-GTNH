...
public class PerkAttributeModifier {
    ...
    protected PerkAttributeModifier getCachedAttributeModifier(PerkConverter converter, String attributeType,
        Mode mode) {
        if (!cachedConverters.containsKey(converter)) {
            cachedConverters.put(converter, HashBasedTable.create());
        }
        Table<String, Mode, PerkAttributeModifier> cachedModifiers = cachedConverters.get(converter);
    ...
    public boolean hasDisplayString() {
        PerkAttributeType type;
        if ((type = resolveType()) != null) {
            return StatCollector.canTranslate(type.getUnlocalizedName());
        }
        return false;
    }
    ...
    public static enum Mode {
        ...
        public static Mode fromVanillaAttributeOperation(int op) {
            return values()[WrapMathHelper.clamp(op, 0, values().length - 1)];
        }

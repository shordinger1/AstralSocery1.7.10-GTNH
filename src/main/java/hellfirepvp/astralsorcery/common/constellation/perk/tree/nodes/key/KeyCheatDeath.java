...
public class KeyCheatDeath extends KeyPerk implements ICooldownPerk {
    ...
    protected void applyEffectMultiplier(double multiplier) {
        ...
        this.potionDuration = WrapMathHelper.ceil(this.potionDuration * multiplier);
        this.potionAmplifier = WrapMathHelper.ceil(this.potionAmplifier * multiplier);
    }
            ...
            if (prog.hasPerkEffect(this) && side == Side.SERVER) {
                if (player.getHealth() <= thresholdApplyPerkHealth || event.ammount >= thresholdApplyPerkDamage) {
                    if (!PerkEffectHelper.EVENT_INSTANCE.isCooldownActiveForPlayer(player, this)) {
                        ...
                        // 1.7.10: PotionEffect constructor takes potion ID (int), not Potion object
                        player.addPotionEffect(
                            new PotionEffect(
                                RegistryPotions.potionCheatDeath.id,
                                potionDuration,
                                potionAmplifier,
                                true));
                    }

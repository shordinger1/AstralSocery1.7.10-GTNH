...
public class GemAttributeModifier extends PerkAttributeModifier {
    ...
    protected PerkAttributeModifier getCachedAttributeModifier(PerkConverter converter, String attributeType,
        Mode mode) {
        if (!gemConverterCache.containsKey(this.getUniqueId())) {
            gemConverterCache.put(this.getUniqueId(), new HashMap<>());
        }
        Map<PerkConverter, Table<String, Mode, PerkAttributeModifier>> modifierCache = gemConverterCache
            .get(this.getUniqueId());
        if (!modifierCache.containsKey(converter)) {
            modifierCache.put(converter, HashBasedTable.create());
        }
        Table<String, Mode, PerkAttributeModifier> cachedModifiers = modifierCache.get(converter);
    ...
    public NBTTagCompound serialize() {
        ...
        NBTHelper.setUniqueId(tag, "id", getUniqueId());
    ...
    public static GemAttributeModifier deserialize(NBTTagCompound tag) {
        UUID id = NBTHelper.getUniqueId(tag, "id");

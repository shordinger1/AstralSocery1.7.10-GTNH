...
public class PlayerChargeHandler implements ITickHandler {
            ...
            if (charge < 1F) {
                if (pl.capabilities.isCreativeMode) {
                    ...
                    float dayMult = ConstellationSkyHandler.getInstance()
                        .getCurrentDaytimeDistribution(pl.worldObj);
                    chargeGain *= (0.5F + dayMult * 0.5F);
                    if (MiscUtils.canSeeSky(pl.worldObj, new BlockPos(pl).up(), false, false)) {
                        chargeGain *= 6F;
                    }
                    charge = WrapMathHelper.clamp(charge + chargeGain, 0F, 1F);
                }
                setCharge(pl, charge);
    ...
    public void setCharge(EntityPlayer player, float charge) {
        chargeMap.put(player, WrapMathHelper.clamp(charge, 0F, 1F));
    }

...
public class WorldSkyHandler {
    ...
    private void setupInitialFunctions() {
        ...
        for (IWeakConstellation c : weakAndMajor) {
            constellations.addFirst(c);
        }
    ...
    private float spSine(int dayStart, int dayIn) {
        ...
        return WrapMathHelper.sin((float) (part * Math.PI)) * 0.25F + 0.75F;
    }
    ...
    private void doConstellationIteration(World w) {
        ...
        if (!initialValueMappings.containsKey(activeDay)) {
            initialValueMappings.put(activeDay, new LinkedList<>());
        }
        ...
        for (IConstellationSpecialShowup special : ConstellationRegistry.getSpecialShowupConstellations()) {
            if (special.doesShowUp(this, w, lastRecordedDay)) {
                ...
                activeDistributions.put(
                    special,
                    WrapMathHelper.clamp(special.getDistribution(this, w, lastRecordedDay, true), 0F, 1F));
            } else {
                activeDistributions.put(
                    special,
                    WrapMathHelper.clamp(special.getDistribution(this, w, lastRecordedDay, false), 0F, 1F));
            }
        }
    }
    ...
    public LinkedList<IConstellation> getSortedActiveConstellations() {
        ...
        Collections.sort(entries, new Comparator<Map.Entry<IConstellation, Float>>() {

            @Override
            public int compare(Map.Entry<IConstellation, Float> e1, Map.Entry<IConstellation, Float> e2) {
                return WrapMathHelper.floor(e2.getValue() * 1000) - WrapMathHelper.floor(e1.getValue() * 1000);
            }
        });
        for (Map.Entry<IConstellation, Float> e : entries) {
            out.addLast(e.getKey());
        }
        return out;
    }
    ...
    public IConstellation getHighestDistributionConstellation(Random random) {
        return getHighestDistributionConstellation(random, new Predicate<IConstellation>() {

            @Override
            public boolean apply(IConstellation c) {
                return true;
            }
        });
    }
    ...
    public IConstellation getHighestDistributionConstellation(Random random, Predicate<IConstellation> acceptorFunc) {
        ...
        for (Map.Entry<IConstellation, Float> entry : activeDistributions.entrySet()) {
            if (entry.getValue() > highest) {
                if (acceptorFunc.apply(entry.getKey())) {
        ...
        if ((highestVal == null || highestVal.size() <= 0)) return null;

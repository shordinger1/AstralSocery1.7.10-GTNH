        ...
        if (!entities.isEmpty()) {
            for (EntityLivingBase e : entities) {
                ...
                if (modified.isCorrupted()) {
                    ...
                    e.addPotionEffect(new PotionEffect(RegistryPotions.potionDropModifier.id, 4000, 5));
                    DamageUtil.attackEntityFrom(e, CommonProxy.dmgSourceStellar, 5000.0F);
                    continue;
                }
                if (rand.nextFloat() < herdChance && MiscUtils.canEntityTickAt(world, new BlockPos(e))) {

...
public class CEffectPelotrio extends CEffectPositionListGen<GenListEntries.PelotrioSpawnListEntry> {
    ...
    // 1.7.10: EntityWitherSkeleton, EntityParrot, EntityStray, EntitySkeletonHorse don't exist
    private static Tuple<Class<Entity>, Class<Entity>>[] swapTable = new Tuple[] {
        new Tuple(EntitySkeleton.class, EntitySkeleton.class), new Tuple(EntityVillager.class, EntityWitch.class),
        new Tuple(EntityPig.class, EntityPigZombie.class), new Tuple(EntityCow.class, EntityZombie.class),
        new Tuple(EntityChicken.class, EntityBlaze.class), new Tuple(EntitySheep.class, EntityCaveSpider.class),
        new Tuple(EntityHorse.class, EntitySkeleton.class) };

    // 1.7.10: AxisAlignedBB doesn't have grow(), calculate manually
    private static AxisAlignedBB proximityCheckBox = AxisAlignedBB.getBoundingBox(-24, -24, -24, 24, 24, 24);
    ...
    public CEffectPelotrio(@Nullable ILocatable origin) {
        super(
            ...
            // 1.7.10: Use Function<BlockPos, PelotrioSpawnListEntry> instead of IGenListFactory
            (BlockPos pos) -> new GenListEntries.PelotrioSpawnListEntry(pos));
    }
    ...
    public boolean playEffect(World world, BlockPos pos, float percStrength, ConstellationEffectProperties modified,
        ...
        if (modified.isCorrupted()) {
            ...
            for (EntityLivingBase e : entities) {
                // 1.7.10: isDead is a field, not a method
                if (e != null && !e.isDead && rand.nextInt(350) == 0) {
                    e = trySwapEntity(world, e);
                    if (e != null) {
                        EntityLivingBase finalEntity = e;
                        // 1.7.10: PotionEffect needs potion id, not Potion object
                        e.addPotionEffect(
                            new PotionEffect(RegistryPotions.potionDropModifier.id, Integer.MAX_VALUE, 3));
                        AstralSorcery.proxy.scheduleDelayed(() -> world.spawnEntityInWorld(finalEntity));
                        did = true;
                    }
                }
            }
        ...
        // 1.7.10: getEntitiesWithinAABB doesn't support lambdas, filter manually
        List<EntityLivingBase> entities = world.getEntitiesWithinAABB(
            EntityLivingBase.class,
            proximityCheckBox.offset(pos.getX(), pos.getY(), pos.getZ()));
        // Filter out dead entities manually (Java 7 doesn't support lambdas)
        java.util.List<EntityLivingBase> filteredList = new java.util.ArrayList();
        for (EntityLivingBase e : entities) {
            if (e != null && !e.isDead) {
                filteredList.add(e);
            }
        }
        if (filteredList.size() > proximityAmount) {

...
public class CEffectHorologium extends CEffectPositionList {
    ...
    public CEffectHorologium(@Nullable ILocatable origin) {
        super(origin, Constellations.horologium, "horologium", maxCount, new CEffectPositionListGen.Verifier() {

            @Override
            public boolean isValid(World world, BlockPos testPos) {
                return TileAccelerationBlacklist
                    .canAccelerate(world.getTileEntity(testPos.getX(), testPos.getY(), testPos.getZ()));
            }
        });
    }
            ...
            if (MiscUtils.isChunkLoaded(world, new ChunkPos(sel))) {
                ...
                if (TileAccelerationBlacklist.canAccelerate(te)) { // Does != null && instanceof ITickable check.
                    ...
                    try {
                        ...
                        while (times > 0) {
                            ((IUpdatePlayerListBox) te).update();

...
public abstract class CEffectPositionList extends CEffectPositionListGen<GenListEntries.SimpleBlockPosEntry> {
    ...
    public CEffectPositionList(@Nullable ILocatable origin, IWeakConstellation c, String cfgName, int maxCount,
        Verifier verifier) {
        super(origin, c, cfgName, maxCount, verifier, new Function<BlockPos, GenListEntries.SimpleBlockPosEntry>() {

            @Override
            public GenListEntries.SimpleBlockPosEntry apply(BlockPos input) {
                return new GenListEntries.SimpleBlockPosEntry(input);
            }
        });
    }

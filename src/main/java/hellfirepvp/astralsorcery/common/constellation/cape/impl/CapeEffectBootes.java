...
public class CapeEffectBootes extends CapeArmorEffect {
    ...
    public void onPlayerTick(EntityPlayer pl) {
        ...
        if (current < maxFlareCount) {
            if (rand.nextFloat() < summonFlareChance) {
                ...
                w.spawnEntityInWorld(flare);
            }
        }
        updateEntityList(w, flare);
        List<Entity> fl = gatherEntities(w);
        for (Entity e : fl) {
            if (e instanceof EntityFlare && ((EntityFlare) e).getFollowingEntity() != null
                && pl.getDistanceSqToEntity(e) >= 100) {
    ...
    public void onPlayerDamagedByEntity(EntityPlayer player, EntityLivingBase attacker) {
        List<Entity> entityFlares = gatherEntities(player.worldObj);
    ...
    private List<Integer> gatherEntityIds() {
        ...
        try {
            // 1.7.10: Use reflection to access NBTTagList's private tagList field
            Field fieldList = NBTTagList.class.getDeclaredField("tagList");
            fieldList.setAccessible(true);
            @SuppressWarnings("unchecked")
            java.util.List<NBTTagInt> tagList = (java.util.List<NBTTagInt>) fieldList.get(list);

            // Also need reflection to access NBTTagInt's private data field
            Field fieldData = NBTTagInt.class.getDeclaredField("data");
            fieldData.setAccessible(true);

            for (NBTTagInt tag : tagList) {
                int value = fieldData.getInt(tag);
                entities.add(value);
            }
        } catch (Exception e) {
            // Silently ignore

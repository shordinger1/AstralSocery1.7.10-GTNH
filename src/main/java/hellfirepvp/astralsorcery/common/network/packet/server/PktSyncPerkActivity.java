...
public class PktSyncPerkActivity implements IMessage, IMessageHandler<PktSyncPerkActivity, IMessage> {
    ...
    public void toBytes(ByteBuf buf) {
        ...
        ByteBufUtils.writeOptional(buf, this.type, new BiConsumer<ByteBuf, Type>() {

            @Override
            public void accept(ByteBuf buf1, Type type) {
                ByteBufUtils.writeEnumValue(buf1, type);
            }
        });
        ByteBufUtils.writeOptional(buf, this.perk, new BiConsumer<ByteBuf, AbstractPerk>() {

            @Override
            public void accept(ByteBuf byteBuf, AbstractPerk perk) {
                ByteBufUtils.writeResourceLocation(byteBuf, perk.getRegistryName());
            }
        });
        ByteBufUtils.writeOptional(buf, this.newData, new BiConsumer<ByteBuf, NBTTagCompound>() {

            @Override
            public void accept(ByteBuf buf1, NBTTagCompound tag) {
                ByteBufUtils.writeNBTTag(buf1, tag);
            }
        });
        ByteBufUtils.writeOptional(buf, this.oldData, new BiConsumer<ByteBuf, NBTTagCompound>() {

...
public class PktPerkGemModification implements IMessageHandler<PktPerkGemModification, IMessage>, IMessage {
    ...
    public void fromBytes(ByteBuf buf) {
        ...
        this.perkKey = ByteBufUtils.readOptional(buf, new Function<ByteBuf, ResourceLocation>() {

            @Override
            public ResourceLocation apply(ByteBuf buf) {
                return ByteBufUtils.readResourceLocation(buf);
            }
        });
    }
    ...
    public void toBytes(ByteBuf buf) {
        ...
        ByteBufUtils.writeOptional(buf, this.perkKey, new BiConsumer<ByteBuf, ResourceLocation>() {

            @Override
            public void accept(ByteBuf buf1, ResourceLocation rl) {
                ByteBufUtils.writeResourceLocation(buf1, rl);
            }
        });
    }
    ...
    public IMessage onMessage(PktPerkGemModification pkt, MessageContext ctx) {
        ...
        switch (pkt.action) {
            case 0:
                ...
                if (!(toInsert == null || toInsert.stackSize <= 0) && toInsert.getItem() instanceof ItemPerkGem
                    ...
                    player.inventory
                        .setInventorySlotContents(pkt.slotId, ItemUtils.copyStackWithSize(stack, stack.stackSize - 1));
                }
                break;
            case 1:

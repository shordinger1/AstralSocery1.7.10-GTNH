...
public class LiquidInteraction {
    ...
    public static LiquidInteraction tryFindInteraction(@Nullable FluidStack fluid1, @Nullable FluidStack fluid2) {
        ...
        for (LiquidInteraction li : registeredInteractions) {
            // 1.7.10: Use getFluid().getID() method
            if (fluid1.getFluid()
                .getID()
                == li.component1.getFluid()
                    .getID()
                && fluid1.amount >= li.component1.amount
                && fluid2.getFluid()
                    .getID()
                    == li.component2.getFluid()
                        .getID()
                && fluid2.amount >= li.component2.amount) {
        ...
        for (LiquidInteraction li : registeredInteractions) {
            if ((li.component1.getFluid()
                ...
                || (li.component2.getFluid()
                    ...
                    && li.component1.getFluid()
                        .equals(comp2))) {
                if (output != null && !(output == null || output.stackSize <= 0)) {
    ...
    public static class InteractionFormBlock extends FluidInteractionAction {
        ...
        public ItemStack getOutputForMatching() {
            return ItemUtils.copyStackWithSize(result, result.stackSize);
        }

        @Override
        public boolean drainComponent1(FluidStack component, IFluidHandler tank) {
            // 1.7.10: IFluidHandler.drain requires ForgeDirection parameter
            FluidStack drained = tank.drain(net.minecraftforge.common.util.ForgeDirection.UNKNOWN, component, false);
            if (drained == null || drained.amount < component.amount) {
                return false;
            }

            if (rand.nextFloat() < c1) {
                drained = tank.drain(net.minecraftforge.common.util.ForgeDirection.UNKNOWN, component, true);

 ...
 *
 * 1.7.10 Compatibility: Uses map-based storage instead of chunk capabilities.
 */
public class FluidRarityRegistry implements ConfigDataAdapter<FluidRarityRegistry.FluidRarityEntry> {
    ...
    // 1.7.10: Map-based chunk data storage
    private static final Map<String, ChunkFluidEntry> CHUNK_DATA_MAP = new ConcurrentHashMap<>();
    ...
    @Nullable
    @Override
    public FluidRarityEntry appendDataSet(String str) {
    ...
    /**
     * 1.7.10: Get chunk entry from map-based storage.
     */
    @Nullable
    public static ChunkFluidEntry getChunkEntry(Chunk ch) {
        String key = getChunkKey(ch);
        return CHUNK_DATA_MAP.get(key);
    }

    /**
     * 1.7.10: Get or create chunk entry.
     */
    private static ChunkFluidEntry getOrCreateChunkEntry(Chunk ch) {
        String key = getChunkKey(ch);
        ChunkFluidEntry entry = CHUNK_DATA_MAP.get(key);
        if (entry == null) {
            entry = new ChunkFluidEntry();
            CHUNK_DATA_MAP.put(key, entry);
        }
        return entry;
    }

    /**
     * 1.7.10: Generate unique key for chunk storage.
     */
    private static String getChunkKey(Chunk ch) {
        // Use world ID and chunk coordinates for unique key
        return ch.worldObj.provider.dimensionId + "_" + ch.xPosition + "_" + ch.zPosition;
    }

    @SubscribeEvent
    public void onChLoad(ChunkDataEvent.Load event) {
        Chunk chunk = event.getChunk();
        ChunkFluidEntry entry = getOrCreateChunkEntry(chunk);

        // Load data from NBT
        NBTTagCompound nbt = event.getData();
        if (nbt.hasKey("AS_ChunkFluidData")) {
            entry.deserializeNBT(nbt.getCompoundTag("AS_ChunkFluidData"));
        } else if (!entry.hadSomeData()) {
            // Generate new data if it doesn't exist
            World w = event.world;
            long seed = w.getSeed();
            long chX = chunk.xPosition;
            long chZ = chunk.zPosition;
            seed ^= chX << 32;
            seed ^= chZ;
    ...
    @SubscribeEvent
    public void onChSave(ChunkDataEvent.Save event) {
        Chunk chunk = event.getChunk();
        String key = getChunkKey(chunk);
        ChunkFluidEntry entry = CHUNK_DATA_MAP.get(key);
        if (entry != null && entry.hadSomeData()) {
            NBTTagCompound nbt = event.getData();
            nbt.setTag("AS_ChunkFluidData", entry.serializeNBT());
        }
    }

...
public class OreTypes implements ConfigDataAdapter<OreEntry> {
    ...
    public ItemStack getRandomOre(Random random) {
        ...
        int runs = 0;
        while ((result == null || result.stackSize <= 0) && runs < 150) {
            ...
            ArrayList<ItemStack> ores = OreDictionary.getOres(key);

            for (ItemStack stack : ores) {
                if ((stack == null || stack.stackSize <= 0) || Block.getBlockFromItem(stack.getItem()) == Blocks.air)
                    continue;
                Item i = stack.getItem();
                // 1.7.10: getNameForObject returns String, not ResourceLocation
                String regName = Item.itemRegistry.getNameForObject(i);
                String regModid = regName != null ? regName.split(":")[0] : "";

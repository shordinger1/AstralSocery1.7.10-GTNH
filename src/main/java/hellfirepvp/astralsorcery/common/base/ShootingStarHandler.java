...
public class ShootingStarHandler implements ITickHandler {
    ...
    public void tick(TickEvent.Type type, Object... context) {
        ...
        if (side == Side.SERVER) {
            ...
            if (w.provider.isHellWorld || w.provider.hasNoSky || !w.provider.isSurfaceWorld()) {
                return;
            }
            ...
            int dayTime = (int) (player.worldObj.getWorldTime() % Config.dayLength);
            if (dayTime >= (midnight - tfHalf) && dayTime <= (midnight + tfHalf)) {
                if (rand.nextInt(ch) == 0) {
                    ...
                    int day = (int) (player.worldObj.getWorldTime() / Config.dayLength);
                    if (!handledDays.contains(day)) {
                        ...
                        EntityShootingStar star = new EntityShootingStar(
                            ...
                            player.posZ,
                            movement);
                        player.worldObj.spawnEntityInWorld(star);
                        handledDays.add(day);

...
public abstract class PatreonPartialEntity {
    ...
    public EntityPlayer findOwner(World world) {
        // 1.7.10: World.getPlayerEntityByUUID() doesn't exist, iterate through players
        for (Object obj : world.playerEntities) {
            if (obj instanceof EntityPlayer) {
                EntityPlayer player = (EntityPlayer) obj;
                if (player.getUniqueID()
                    .equals(this.ownerUUID)) {
                    return player;
                }
            }
        }
    ...
    public void readFromNBT(NBTTagCompound cmp) {
        ...
        this.lastTickedDim = cmp.hasKey("lastTickedDim") ? cmp.getInteger("lastTickedDim") : null;
        this.partialEntityUUID = NBTHelper.getUniqueId(cmp, "uniqueId");
    ...
    public void writeToNBT(NBTTagCompound cmp) {
        ...
        NBTHelper.setUniqueId(cmp, "uniqueId", this.partialEntityUUID);
        if (updatePos) {

...
public class ItemPerkGem extends Item implements IItemVariants {
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip) {
        ...
        if (prog.wasOnceAttuned()) {
            for (GemAttributeModifier mod : getModifiers(stack)) {
                tooltip.add(
                    EnumChatFormatting.GRAY.toString() + EnumChatFormatting.ITALIC + mod.getLocalizedDisplayString());
            }
        } else {
    ...
    public static List<GemAttributeModifier> getModifiers(ItemStack stack) {
        if ((stack == null || stack.stackSize <= 0) || !(stack.getItem() instanceof ItemPerkGem)) {
    ...
    public String getUnlocalizedName(ItemStack stack) {
        ...
        if (i instanceof ItemPerkGem) {
            GemType type = GemType.values()[WrapMathHelper.clamp(stack.getItemDamage(), 0, GemType.values().length)];
